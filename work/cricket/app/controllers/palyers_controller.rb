class PalyersController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @palyer.save
        ...
        format.json { render action: 'show', status: :created, location: @palyer }
      else
  ...
  def destroy
    @palyer.destroy
    respond_to do |format|
      format.html { redirect_to palyers_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def palyer_params
      params.require(:palyer).permit(:player_name, :score, :team_id)
    end
end
